...
(function (window, document, $, Drupal) {
    ...
    Drupal.behaviors.tesla_user = {
        ...
        submitCreateAcctForm: function (origin) {
            ...
            if (origin) {
                ...
                if (password !== password_confirm) {
                    error_msg = Drupal.t('Passwords do not match.');
                    form_error = 1;
                }
            }
        ...
        checkResetPasswordFields: function (action) {
            ...
            var requestUrl       = $('#edit-request_url').val();
            var match            = /user\/reset\/\d+\/(\d+)\/([\w\-=]+)/.exec(requestUrl)
            var timestamp        = match[1];
            var hash             = match[2];
            var destination      = getParameterByName('destination', requestUrl);
